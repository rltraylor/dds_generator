//unified_lcd_8bit.c
//A set of useful functions for writing to a 2x16 LCD characer display
//includes adaptation for 4 bit LCD interface.
...
// after the byte is sent.  Zero delays are ok but still incurr a 4us delay. 
//
...
uint8_t temp;
  ...
  delay_us(wait);                     //typ 1ms for CMDs, 100uS for CHARs  TODO: KLUDGE ALERT
#else //4-bit mode
...
//function more often than every 1.5ms which is the maximum time 
//it might take for any operation to take in this function.
//
...
#else //4-bit mode
    send_lcd(CHAR_BYTE, lcd_string_array[index], 0);
#endif
    index++;        //increment to next character
    if(index == 16) {_delay_us(40); //char just sent, must wait 
                    home_line2(0);}  //on to line 2,   1st position 
    if(index == 32) {_delay_us(40); //char just sent, must wait 
                    cursor_home(0); 
                    index=0;} //back to line 1, 1st position
}//refresh_lcd
/***********************************************************************/
...
void cursor_home(uint8_t delay){send_lcd(CMD_BYTE, 0x02, delay);} //1.5ms wait for LCD
  
//-----------------------------------------------------------------------------
//                          home_line2    
//
//Put cursor at row 1, column 0
void home_line2(uint8_t delay){send_lcd(CMD_BYTE, 0xC0, delay);} //1.5ms wait for LCD
...
void lcd_init(void){
  ...
  LCD_PORT_DDR |= 0xF0                   | //initalize data pins   
                 ((1<<LCD_CMD_DATA_BIT)  | //initalize control pins
                  (1<<LCD_STROBE_BIT  )  |
                  (1<<LCD_RDWR_BIT)    ); 
  //do first four writes in 8-bit mode assuming reset by instruction command 
  //makes some assumptions here, be careful and recheck
  //write asserted as its initalized to zero
  //bug is here  .......
  temp = LCD_PORT & 0x0F;    //perserve lower nibble, clear top nibble
  LCD_PORT = temp | 0x30; strobe_lcd(); _delay_ms(8);    //function set 
  LCD_PORT = temp | 0x30; strobe_lcd(); _delay_us(200);  //function set 
  LCD_PORT = temp | 0x30; strobe_lcd(); _delay_us(80);    //function set 
  LCD_PORT = temp | 0x20; strobe_lcd(); _delay_us(80);   //set 4-bit mode
                ...
                uint8_t bZeroFill)  //non-zero if a specified fieldwidth is to be zero filled
{
      ...
      do{send_lcd(CHAR_BYTE, sline[--i] , 100);} while(i);
